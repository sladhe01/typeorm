    ...
    implements WhereExpressionBuilder
{
    ...
    /**
     * Gets entities returned by execution of generated query builder sql with aliases.
     * This method differs from getMany only in its return type, providing enhanced flexibility in type handling.
     */
    async getManyWithAlias(): Promise<any[]> {
        if (this.expressionMap.lockMode === "optimistic")
            throw new OptimisticLockCanNotBeUsedError()
            ...
            const querySelects = metadata.primaryColumns.map(
                (primaryColumn) => {
                    ...
                    const selectedColumn = this.expressionMap.selects.find(
                        (select) =>
                            select.selection ===
                            mainAliasName + "." + primaryColumn.databaseName,
                    )
                    const columnAlias =
                        selectedColumn && selectedColumn.aliasName
                            ? this.escape(selectedColumn.aliasName)
                            : this.escape(
                                  DriverUtils.buildAlias(
                                      this.connection.driver,
                                      undefined,
